...
git clone https://github.com/sandraschi/glance-mcp
cd glance-mcp
just
```
...
4. Start the server:
   ...
   uv run python -m glance_mcp.server

   # HTTP mode (for web dashboard)
   uv run uvicorn glance_mcp.server:app --port 10776
   ```
5. Open `http://localhost:10776` or the frontend URL.
...
| Something else | [Open a GitHub issue](https://github.com/sandraschi/glance-mcp/issues) |
